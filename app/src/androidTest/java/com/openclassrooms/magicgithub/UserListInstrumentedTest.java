...
public class UserListInstrumentedTest {
    ...
    @Before // effectue avant d effectuer les tests
    public void setup() {
    ...
    public void checkIfAddingRandomUserIsWorking() {
        onView(withId(R.id.activity_list_user_fab)).perform(click()); // aller ds res (ressources) le test clic sur le bouton
        onView(withId(R.id.activity_list_user_rv)).check(new RecyclerViewUtils.ItemCount(currentUsersSize +1)); // cpte nbre user ds la vue ; 1
    }
